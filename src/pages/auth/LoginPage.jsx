    ...
    return (
        <div className="min-h-screen bg-[var(--color-bg-dark)] text-white flex items-center justify-center p-4 relative overflow-hidden">
            ...
            <div className="max-w-md w-full relative z-10">
                ...
                <Card className="p-1 border-[var(--color-border)] bg-[var(--color-bg-card)]/40 backdrop-blur-2xl rounded-[2.5rem] shadow-2xl overflow-hidden mt-2 border-t-white/10">
                    {/* Tabs */}
                    <div className="flex p-1.5 bg-[var(--color-bg-dark)]/80 rounded-t-[2.3rem] mb-4 gap-1">
                        {[
                            ...
                            <button
                                key={tab.id}
                                onClick={() => { setAuthMode(tab.id); setError(''); setIsSignup(false); }}
                                className={cn(
                    ...
                    <div className="px-6 pb-10 pt-4">
                        ...
                        {authMode === 'email' && (
                            <form onSubmit={isForgotPassword ? handleForgotPassword : handleEmailAuth} className="space-y-5">
                                ...
                                <div className="space-y-2">
                                    <label className="text-[9px] font-black uppercase tracking-[0.2em] text-[var(--color-text-gray)] ml-1 opacity-60">Security Email</label>
                                    <div className="relative group">
                                        <Mail className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--color-text-gray)] group-focus-within:text-[var(--color-primary)] transition-colors" />
                                        <input
                                            type="email"
                                            value={email}
                                    ...
                                    <>
                                        <div className="space-y-2">
                                            <div className="flex justify-between items-center ml-1">
                                                <label className="text-[9px] font-black uppercase tracking-[0.2em] text-[var(--color-text-gray)] opacity-60">Passcode</label>
                                                {!isSignup && (
                                                    <button
                                                        type="button"
                                                        onClick={() => setIsForgotPassword(true)}
                                                        className="text-[9px] text-[var(--color-primary)] font-black uppercase tracking-widest hover:underline"
                                                    >
                                                        Forgot?
                                                    </button>
                                                )}
                                            ...
                                            <div className="relative group">
                                                <Key className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--color-text-gray)] group-focus-within:text-[var(--color-primary)] transition-colors" />
                                                <input
                                                    type={showPassword ? "text" : "password"}
                                                    value={password}
                                                    onChange={(e) => setPassword(e.target.value)}
                                                    className="w-full bg-[var(--color-bg-dark)]/50 border border-[var(--color-border)] rounded-2xl h-14 pl-12 pr-12 focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)] outline-none text-sm font-bold transition-all shadow-inner"
                                                    placeholder="••••••••"
                                                    required
                                                />
                                                <button
                                        ...
                                        {isSignup && (
                                            <div className="space-y-2">
                                                <label className="text-[9px] font-black uppercase tracking-[0.2em] text-[var(--color-text-gray)] ml-1 opacity-60">Repeat Passcode</label>
                                                <div className="relative group">
                                                    <Key className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--color-text-gray)] group-focus-within:text-[var(--color-primary)] transition-colors" />
                                                    <input
                                    ...
                                    {loadingAction ? (
                                        ...
                                        <>
                                            {isForgotPassword
                                                ...
                                                : (isSignup ? <UserPlus className="h-4 w-4 mr-3" /> : <LogIn className="h-4 w-4 mr-3" />)}
                                            {isForgotPassword
                                                ? 'Send Link'
