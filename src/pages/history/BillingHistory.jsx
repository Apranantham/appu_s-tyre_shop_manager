...
const BillingHistory = () => {
    ...
    const [viewMode, setViewMode] = useState('invoices'); // 'invoices', 'payments'
    ...
    const getSortedInvoices = () => {
        const filtered = invoices.filter(inv => {
            ...
            if (dateFilter === 'today') {
                ...
                if (invDate.getMonth() !== lastMonthDate.getMonth() || invDate.getFullYear() !== lastMonthDate.getFullYear()) return false;
            } else if (dateFilter === 'customMonth') {
                const [year, month] = customMonth.split('-').map(Number);
                if (invDate.getFullYear() !== year || invDate.getMonth() !== (month - 1)) return false;
            }
    ...
    const getFilteredPayments = () => {
        ...
        invoices.forEach(inv => {
            ...
            // Synthesis logic: If paidAmount > recorded payments, synthesize the initial payment
            ...
            if ((inv.paidAmount || 0) > recordedPaymentsTotal) {
                ...
                synthesizedPayments.push({
                    amount: diff,
                    date: inv.date, // Use invoice date for initial payment
                    mode: inv.paymentMode || 'cash',
                    note: lang === 'ta' ? 'தொடக்கத் தொகை (தானியங்கி)' : 'Initial payment (Auto-sync)',
                    isSynthesized: true
                });
            }

            [...synthesizedPayments, ...recordedPayments].forEach(p => {
                allPayments.push({
                    ...p,
                    invoiceId: inv.id,
        ...
        const filtered = allPayments.filter(p => {
            ...
            const invNo = (String(p.invoiceNo) || '').toLowerCase();
            const search = searchTerm.toLowerCase();

            const matchesSearch = name.includes(search) || invNo.includes(search);
            if (!matchesSearch) return false;
            ...
            if (dateFilter === 'today') {
                ...
                const lastMonthDate = new Date(now.getFullYear(), now.getMonth() - 1, 1);
                if (pDate.getMonth() !== lastMonthDate.getMonth() || pDate.getFullYear() !== lastMonthDate.getFullYear()) return false;
            } else if (dateFilter === 'customMonth') {
                const [year, month] = customMonth.split('-').map(Number);
                if (pDate.getFullYear() !== year || pDate.getMonth() !== (month - 1)) return false;
            }

            if (activeUserFilter) {
                if (p.createdBy !== activeUserFilter) return false;
            }
...
Thank you for your business! 🏁`;
    ...
    return (
        <div className="space-y-6 pb-20">
            ...
            <div className="flex bg-[var(--color-bg-dark)] p-1 rounded-2xl border border-[var(--color-border)] shadow-inner w-fit">
                <button
                ...
                >
                    ...
                    {lang === 'ta' ? 'வரவுகள்' : 'Payments'}
                </button>
            </div>

            {/* Date Filters Bar */}
            <div className="flex flex-wrap items-center gap-3">
                <div className="flex bg-[var(--color-bg-dark)] p-1 rounded-2xl border border-[var(--color-border)] overflow-x-auto no-scrollbar shadow-inner max-w-full">
                    {[
                        ...
                        <button
                            ...
                            className={cn(
                                "px-4 py-2 rounded-xl text-[8px] font-black uppercase tracking-widest whitespace-nowrap transition-all",
                ...
                <div className={cn(
                    "flex items-center gap-2 bg-[var(--color-bg-dark)] p-1 pr-3 rounded-2xl border transition-all",
                    dateFilter === 'customMonth' ? "border-[var(--color-primary)] bg-[var(--color-primary)]/5" : "border-[var(--color-border)] opacity-60 hover:opacity-100"
                )}>
                    <button
                        onClick={() => setDateFilter('customMonth')}
                        className={cn(
                            "px-4 py-2 rounded-xl text-[8px] font-black uppercase tracking-widest whitespace-nowrap transition-all",
                            dateFilter === 'customMonth' ? "bg-[var(--color-primary)] text-white shadow-md shadow-blue-500/10" : "text-[var(--color-text-gray)]"
                        )}
                    >
                        {t.any_month}
                    </button>
                    <input
                        ...
                        className="bg-transparent text-[10px] font-black text-[var(--color-primary)] focus:outline-none cursor-pointer uppercase color-scheme-dark-light"
            ...
            <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center">
                <div className="relative flex-1">
                    ...
                    <input
                        placeholder={viewMode === 'payments'
                            ? (lang === 'ta' ? 'தேடு (பில் #, பெயர்)...' : 'Search Payments (Bill #, Name)...')
                            : (lang === 'ta' ? 'தேடு (பில் #, பெயர், போன்)...' : 'Search (Bill #, Name, Phone)...')}
                        value={searchTerm}
                ...
                {viewMode === 'invoices' ? (
                    ...
                    <div className="flex items-center space-x-3 px-6 py-3 bg-[var(--color-primary)]/10 rounded-2xl border border-[var(--color-primary)]/20 shadow-sm animate-in fade-in zoom-in-95 duration-200">
                        <div className="flex flex-col">
                            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-[var(--color-primary)] opacity-70 leading-none mb-1">{lang === 'ta' ? 'மொத்த வரவு' : 'Total Revenue'}</span>
                            <span className="text-xl font-black text-[var(--color-text-white)] leading-none">₹{totalPaymentsReceived.toLocaleString()}</span>
                        </div>
                        <div className="h-8 w-px bg-[var(--color-primary)]/20 mx-2"></div>
                        <div className="flex flex-col">
                            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-[var(--color-text-gray)] leading-none mb-1">{lang === 'ta' ? 'உருப்படிகள்' : 'Entries'}</span>
                            <span className="text-sm font-bold text-[var(--color-text-gray)] leading-none">{displayPayments.length}</span>
                        </div>
                    </div>
                )}

                {viewMode === 'invoices' && (
                    <div className="flex items-center space-x-3 px-6 py-3 bg-[var(--color-bg-dark)]/50 rounded-2xl border border-[var(--color-border)] shadow-sm animate-in fade-in zoom-in-95 duration-200">
                        <div className="flex flex-col">
                            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-[var(--color-text-gray)] opacity-60 leading-none mb-1">{lang === 'ta' ? 'மொத்த பில்' : 'Total Billed'}</span>
                            <span className="text-xl font-black text-[var(--color-text-white)] leading-none">₹{displayInvoices.reduce((sum, inv) => sum + (inv.total || 0), 0).toLocaleString()}</span>
                        </div>
                        <div className="h-8 w-px bg-[var(--color-border)] mx-2"></div>
                        <div className="flex flex-col">
                            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-[var(--color-text-gray)] leading-none mb-1">{lang === 'ta' ? 'பில்கள்' : 'Bills'}</span>
                            <span className="text-sm font-bold text-[var(--color-text-gray)] leading-none">{displayInvoices.length}</span>
                        </div>
                    </div>
                )}
            </div>
                                ...
                                <div key={inv.id} className="p-5 space-y-5 bg-[var(--color-bg-card)] hover:bg-[var(--color-bg-dark)]/50 transition-colors active:scale-[0.99] duration-200" onClick={() => { setSelectedInvoice(inv); setShowPreview(true); }}>
                                    <div className="flex justify-between items-start">
                                        <div className="space-y-1.5">
                                            ...
                                            <p className="text-[9px] font-bold uppercase tracking-widest text-gray-500 truncate max-w-[120px]">{inv.customer?.vehicle || 'N/A'}</p>
                                        </div>
                                    ...
                                    <div className="flex items-center justify-between gap-3">
                                        <div className="flex gap-2 flex-1">
                                            ...
                                            <button
                                                onClick={(e) => { e.stopPropagation(); setSelectedInvoice(inv); setShowPreview(true); }}
                                                className="w-12 h-11 flex items-center justify-center bg-[var(--color-bg-dark)] text-gray-400 rounded-2xl border border-[var(--color-border)] active:scale-95 transition-all"
                                            >
                    ...
                    <>
                        ...
                        <div className="hidden md:block overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                <thead className="bg-[var(--color-bg-dark)] text-[var(--color-text-gray)] text-[10px] uppercase tracking-wider font-bold">
                                    <tr>
                                        <th className="px-6 py-4">{lang === 'ta' ? 'தேதி' : 'Payment Date'}</th>
                                        <th className="px-6 py-4">{lang === 'ta' ? 'பில் #' : 'Invoice #'}</th>
                                        <th className="px-6 py-4">{t.customers}</th>
                                        <th className="px-6 py-4">{lang === 'ta' ? 'முறை' : 'Mode'}</th>
                                        <th className="px-6 py-4">{lang === 'ta' ? 'தொகை' : 'Amount'}</th>
                                        <th className="px-6 py-4 text-right"></th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-[var(--color-border)]">
                                    {displayPayments.length === 0 ? (
                                        <tr>
                                            <td colSpan="6" className="px-6 py-12 text-center text-[var(--color-text-gray)]">
                                                <Coins className="h-10 w-10 mx-auto mb-2 opacity-10" />
                                                {lang === 'ta' ? 'வரவுகள் எதுவும் இல்லை' : 'No payments found'}
                                            </td>
                                        </tr>
                                    ) : (
                                        displayPayments.map((p, idx) => (
                                            <tr key={`${p.invoiceId}-${idx}`} className="hover:bg-[var(--color-bg-dark)] transition-colors group">
                                                <td className="px-6 py-4">
                                                    <p className="font-bold text-sm tracking-tight text-[var(--color-text-white)]">
                                                        {new Date(p.date).toLocaleString(lang === 'ta' ? 'ta-IN' : 'en-IN', {
                                                            day: '2-digit',
                                                            month: 'short',
                                                            year: 'numeric',
                                                            hour: '2-digit',
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    <button
                                                        onClick={() => { setSelectedInvoice(p.parentInvoice); setShowPreview(true); }}
                                                        className="p-2 hover:bg-[var(--color-bg-dark)] rounded-lg text-[var(--color-text-gray)] hover:text-[var(--color-primary)] transition-colors"
                                                    >
                                                        <Eye className="h-4 w-4" />
                                                    </button>
                            ...
                            {displayPayments.map((p, idx) => (
                                <div key={`${p.invoiceId}-${idx}`} className="p-5 bg-[var(--color-bg-card)] hover:bg-[var(--color-bg-dark)]/50 transition-colors" onClick={() => { setSelectedInvoice(p.parentInvoice); setShowPreview(true); }}>
                                    <div className="flex justify-between items-center">
                                        <div>
                                            <p className="text-[9px] font-black text-[var(--color-primary)] uppercase tracking-widest mb-1">#{p.invoiceNo}</p>
                                            <h3 className="font-black text-sm text-[var(--color-text-white)]">{p.customer?.name}</h3>
                                            <p className="text-[10px] text-[var(--color-text-gray)] mt-1">
                                                {new Date(p.date).toLocaleString([], { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}
                                            </p>
                                        </div>
                                        <div className="text-right">
